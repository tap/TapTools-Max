...
class vocoder : public object<vocoder>, public sample_operator<2, 1> {
  public:
    ...
    attribute<number> q{this, "q", 20.0, range{0.5, 200.0}, setter{MIN_FUNCTION{m_q = args[0];
    recalc_filters();
    return {m_q};
}
}
, description {
    "The Q factor (resonance) shared by all of the bandpass filters. Higher values "
    "give narrower, more 'robotic' bands."
}
}
;

attribute<number> response_interval{this, "response_interval", 20.0, range{0.1, 1000.0},
                                    setter{MIN_FUNCTION{m_response_ms = args[0];
recalc_envelope();
return {m_response_ms};
}
}
, description {
    "The envelope-follower analysis period, in milliseconds. Shorter values track "
...
message<> dspsetup{this, "dspsetup", "Recompute coefficients when the DSP chain starts.", MIN_FUNCTION{recalc_filters();
recalc_envelope();
return {};
}
}
