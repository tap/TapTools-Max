...
class svf : public object<svf>, public vector_operator<> {
  private:
    ...
    int effective_order() const { return kernel::is_eq_mode(m_filter.mode()) ? 2 : m_filter.order(); }
  ...
  public:
    ...
    svf(const atoms& args = {}) { m_filter.prepare(samplerate(), 1); }

    attribute<symbol> type{
        this,
        "type",
        "lowpass",
        range{"lowpass", "highpass", "bandpass", "notch", "peak", "allpass", "morph", "bell", "lowshelf", "highshelf"},
        setter{ MIN_FUNCTION {
            int m = kernel::mode_lowpass;
            if (args[0] == "highpass") {
                m = kernel::mode_highpass;
            }
            else if (args[0] == "bandpass") {
                m = kernel::mode_bandpass;
            }
            ...
            else if (args[0] == "allpass") {
                m = kernel::mode_allpass;
            }
            ...
            else if (args[0] == "lowshelf") {
                m = kernel::mode_lowshelf;
            }
            else if (args[0] == "highshelf") {
                m = kernel::mode_highshelf;
            }
            m_filter.set_mode(m);
            return args;
        }},
...
#undef TAP_SVF_ATTR
    ...
    attribute<int> order{
        this, "order", 2, setter{ MIN_FUNCTION {
            const int v = (static_cast<int>(args[0]) >= 8) ? 8 : (static_cast<int>(args[0]) >= 4) ? 4 : 2;
    ...
    message<> q{this, "q",
                ...
                MIN_FUNCTION {
                    if (args.empty()) {
                        cout << "q: "
                             << kernel::q_from_resonance(m_filter.param(kernel::p_resonance), effective_order())
                             << endl;
                    }
                    else {
