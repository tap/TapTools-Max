...
class svf : public object<svf>, public vector_operator<> {
  private:
    ...
    static constexpr double k_twopi{6.28318530717958647692};
  ...
  public:
    ...
    attribute<symbol> type{this, "type", "bandpass", range{"lowpass", "highpass", "bandpass", "notch", "peak"},
                           setter{MIN_FUNCTION{if (args[0] == "lowpass"){m_mode = mode_lowpass;
} else if (args[0] == "highpass") {
    m_mode = mode_highpass;
}
else if (args[0] == "bandpass") {
    m_mode = mode_bandpass;
}
else if (args[0] == "notch") {
    m_mode = mode_notch;
}
else if (args[0] == "peak") {
    m_mode = mode_peak;
...
, description {
    "Filter type: lowpass, highpass, bandpass, notch, or peak."
}
}
;

attribute<number> frequency{this, "frequency", 0.0, setter{MIN_FUNCTION{m_frequency = args[0];
...
, description {
    "Cutoff / center frequency in Hz (ramped via portamento)."
}
}
;

attribute<number> resonance{this, "resonance", 1.0, setter{MIN_FUNCTION{m_res = static_cast<double>(args[0]) * 0.1;
return args;
}
}
, description {
...
attribute<number> lfo_freq_depth{this, "lfo.freq.depth", 1.0, setter{MIN_FUNCTION{m_lfo_depth = args[0];
if (m_lfo_depth > (m_frequency - 1.0)) {
    m_lfo_depth = m_frequency - 1.0;
}
return {m_lfo_depth};
...
, description {
    "Depth (in Hz) of the LFO modulation applied to the cutoff frequency."
}
}
;

attribute<number> lfo_freq_speed{this, "lfo.freq.speed", 0.0,
                                 setter{MIN_FUNCTION{m_lfo_speed = MIN_CLAMP(static_cast<double>(args[0]), 0.0, 200.0);
return {m_lfo_speed};
}
}
, description {
    "Speed (in Hz) of the cutoff-frequency LFO."
}
}
;

attribute<symbol> lfo_freq_shape{this, "lfo.freq.shape", "sine", range{"sine", "triangle", "square", "ramp"},
                                 setter{MIN_FUNCTION{if (args[0] == "triangle"){m_lfo_shape = shape_triangle;
}
else if (args[0] == "square") {
    m_lfo_shape = shape_square;
}
else if (args[0] == "ramp") {
    m_lfo_shape = shape_ramp;
}
else {
    m_lfo_shape = shape_sine;
}
return args;
}
}
, description {
    "LFO waveform: sine, triangle, square, or ramp."
}
}
;

attribute<bool> lfo_freq_toggle{this, "lfo.freq.toggle", false, description{"Enable the cutoff-frequency LFO."}};

attribute<number> portamento_time{this, "portamento_time", 5.0, setter{MIN_FUNCTION{m_porta_ms = args[0];
return args;
}
}
, description {
    "Glide time (ms) applied to frequency changes."
}
}
;

message<> clear{this, "clear", "Reset the filter's internal state.", MIN_FUNCTION{m_l.clear();
m_r.clear();
return {};
}
}
...
message<> dspsetup{this, "dspsetup", "Recompute rates when the DSP chain starts.", MIN_FUNCTION{m_l.clear();
m_r.clear();
update_portamento_step();
return {};
}
...
void set_cutoff(double cf) {
    ...
    double       freq = 2.0 * std::sin(k_pi * fc / (sr * 2.0));
    if (freq > 0.25) {
        freq = 0.25;
    }
    m_freq_coef = freq;
...
double lfo_next() {
    ...
    switch (m_lfo_shape) {
    case shape_square:
        w = (m_lfo_phase < 0.5) ? 1.0 : -1.0;
        break;
    case shape_ramp:
        w = -1.0 + 2.0 * m_lfo_phase;
        break;
    case shape_triangle: // 0 → +1 → 0 → -1 → 0
        if (m_lfo_phase < 0.25) {
            w = 4.0 * m_lfo_phase;
        }
        else if (m_lfo_phase < 0.75) {
            w = 2.0 - 4.0 * m_lfo_phase;
        }
        else {
            w = 4.0 * m_lfo_phase - 4.0;
        }
        break;
    case shape_sine:
    default:
        w = std::sin(k_twopi * m_lfo_phase);
        break;
    }
