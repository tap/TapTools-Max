...
class filter : public object<filter>, public vector_operator<> {
  ...
  public:
    ...
    attribute<symbol> mode{this, "mode", "lowpass",
                           range{"lowpass", "highpass", "bandpass", "bandpass_constant_peak", "notch", "allpass",
                                 "peaking", "lowshelf", "highshelf"},
                           setter{MIN_FUNCTION{m_dirty = true;
    return args;
}
}
, description {
    "The filter type. One of: lowpass, highpass, bandpass, "
...
attribute<number> frequency{this, "frequency", 1000.0,
                            setter{MIN_FUNCTION{m_freq = std::max(0.0, static_cast<double>(args[0]));
m_dirty = true;
return {m_freq};
}
}
, description {
    "Cutoff / center frequency of the filter, in Hz."
}
}
;

attribute<number> q{this, "q", 0.707, setter{MIN_FUNCTION{m_q = std::max(1e-6, static_cast<double>(args[0]));
m_dirty = true;
return {m_q};
}
}
, description {
    "The Q (resonance / narrowness / steepness) of the filter. For the shelf "
    "types this is reused as the shelf slope S."
}
}
;

attribute<number> gain{this, "gain", 0.0, setter{MIN_FUNCTION{m_gain = args[0];
m_dirty = true;
return args;
}
}
, description {
    "Gain in decibels, applied by the peaking, lowshelf, and highshelf types."
}
}
;

message<> clear{this, "clear", "Reset the filter's internal state (flush the delay memory).", MIN_FUNCTION{m_z1 = 0.0;
m_z2 = 0.0;
return {};
}
}
...
message<> dspsetup{this, "dspsetup", "Recompute coefficients when the DSP chain starts.", MIN_FUNCTION{m_z1 = 0.0;
m_z2      = 0.0;
...
coeffs compute(double f0) const {
    ...
    const double d_bgain = m_gain;
    const double Q       = m_q;
    const double S       = m_q; // q doubles as the shelf slope, as in tap.biquadcalc

    const double A   = std::pow(10.0, d_bgain / 40.0);
    const double w0  = k_twopi * f0 / Fs;
