...
namespace {
    ...
    constexpr double k_c_tau    = 6.28318530717958647692;
    constexpr long   k_c_period = 96; // input sine period in samples
    ...
    std::vector<double> run_sine(shift& object, size_t n, double amp = 0.5) {
        ...
        for (size_t i = 0; i < n; ++i) {
            out[i] = object(amp * std::sin(k_c_tau * static_cast<double>(i) / k_c_period));
        }
        return out;
    }
...
SCENARIO("tap.shift~ transposes by the given ratio") {
    ext_main(nullptr);
    const double w = k_c_tau / k_c_period;
...
SCENARIO("parameter changes glide without clicks") {
    ...
    GIVEN("a running sine and a drastic window_size change") {
        ...
        for (size_t i = 0; i < settle; ++i) {
            prev = my_object(0.5 * std::sin(k_c_tau * static_cast<double>(i) / k_c_period));
        }
        ...
        for (size_t i = settle; i < settle + settle; ++i) {
            const double y = my_object(0.5 * std::sin(k_c_tau * static_cast<double>(i) / k_c_period));
