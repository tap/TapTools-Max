...
public:
    ...
    attribute<number> window_size { this, "window_size", 87.0,
        setter { MIN_FUNCTION {
            const double v = MIN_CLAMP(static_cast<double>(args[0]), 1.0, c_buffersize_ms - 6.0);
            set_ramp(m_window, v);
            return { v };
        }},
    ...
    attribute<number> smooth { this, "smooth", 20.0,
        setter { MIN_FUNCTION {
    ...
    sample operator()(sample x) {
        ...
        const double s   = std::sin(c_pi * ph);
        const double e1  = s * s;
        const double e2  = 1.0 - e1;

        double y = 0.0;
        if (e1 > 0.0)
            y += e1 * read_hermite(c_base_delay + ph * window_samples, N);
        if (e2 > 0.0)
            y += e2 * read_hermite(c_base_delay + ph2 * window_samples, N);
...
private:
    ...
    static constexpr double c_base_delay    { 3.0 };   // headroom so Hermite never reads the write point
    ...
    void allocate() {
        const long n = std::max(16L, static_cast<long>(c_buffersize_ms * samplerate() * 0.001));
