...
std::vector<double> saw_riff(double seconds, double amp = 0.35) {
    ...
    std::vector<double> sig(static_cast<size_t>(seconds * g_sr), 0.0);
    double phase = 0.0;
...
int main(int argc, char** argv) {
    ...
    // 1. The classic: resonant sweep over a saw riff.
    {
        ...
        write_wav(dir + "ladder_sweep_saw.wav", render(f, saw_riff(10.0)), g_sr);
    }
    ...
    // 2. Self-oscillation synth voice: ping it, then play the cutoff per sample.
    {
        ...
        const size_t n = static_cast<size_t>(8.0 * g_sr);
        std::vector<double> out(n);
        for (size_t i = 0; i < n; ++i) {
            const double t = i / g_sr;
            const int    step = static_cast<int>(t) % 8;
            const double ping = (std::fmod(t, 1.0) < 1.0 / g_sr) ? 0.4 : 0.0;  // one ping per note
            out[i] = f.process(ping, melody[step]);
        }
        write_wav(dir + "ladder_selfosc_melody.wav", out, g_sr);
    }
    ...
    // 3. Drive A/B: four bars clean, four bars +18 dB into the ladder.
    {
        ...
        write_wav(dir + "ladder_drive_ab.wav", out, g_sr);
    }
    ...
    // 4. Mode tour: lp24 -> lp12 -> bp12 -> bp24 -> hp12 -> hp24, ~1.6 s each.
    {
        ...
        write_wav(dir + "ladder_mode_tour.wav", out, g_sr);
    }
    ...
    // 5. Preset morph: dark and heavy -> open and screaming over 4 s.
    {
        ...
        write_wav(dir + "ladder_preset_morph.wav", out, g_sr);
    }
