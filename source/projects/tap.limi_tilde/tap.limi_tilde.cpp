...
class limi : public object<limi>, public vector_operator<> {
  public:
    ...
    attribute<number> threshold{this, "threshold", 0.0, setter{MIN_FUNCTION{
                                    m_threshold = db_to_amp(args[0]);
                                    return args;
                                }},
                                description{"Limiting threshold in decibels."}};

    attribute<number> preamp{this, "preamp", 0.0, setter{MIN_FUNCTION{
                                 m_preamp = db_to_amp(args[0]);
                                 return args;
                             }},
                             description{"Gain (dB) applied before limiting."}};

    attribute<number> postamp{this, "postamp", 0.0, setter{MIN_FUNCTION{
                                  m_postamp = db_to_amp(args[0]);
                                  return args;
                              }},
                              description{"Gain (dB) applied after limiting."}};

    attribute<number> release{this, "release", 1000.0, setter{MIN_FUNCTION{
                                  m_release = args[0];
                                  set_recover();
                                  return args;
                              }},
    ...
    attribute<int> lookahead{this, "lookahead", 100, setter{MIN_FUNCTION{
                                 m_lookahead = std::clamp(static_cast<int>(args[0]), 1, k_max_samples - 1);
