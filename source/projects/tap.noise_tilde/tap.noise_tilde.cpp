...
class noise : public object<noise>, public sample_operator<0, 1> {
  ...
  public:
    ...
    attribute<symbol> mode{this, "mode", "white", range{"white", "pink", "brown", "blue", "gauss"},
                           setter{MIN_FUNCTION{if (args[0] == "pink"){m_color = pink;
...
attribute<number> gain{this, "gain", 0.0,
                       setter{MIN_FUNCTION{m_gain = std::pow(10.0, static_cast<double>(args[0]) * 0.05); // dB -> linear
return args;
}
}
, description {
    "Output gain in decibels (does not apply to gaussian noise)."
}
}
;

attribute<number> mean{this, "mean", 0.0, setter{MIN_FUNCTION{m_mean = args[0];
rebuild_normal();
return args;
}
}
...
attribute<number> deviation{this, "deviation", 1.0, range{0.0, 1.0},
                            setter{MIN_FUNCTION{m_deviation = MIN_CLAMP(static_cast<double>(args[0]), 0.0, 1.0);
rebuild_normal();
return args;
}
}
...
message<> clear{this, "clear", "Reset the noise filter memory.",
                MIN_FUNCTION{m_b0 = m_b1 = m_b2 = m_b3 = m_b4 = m_b5 = m_b6 = 0.0;
m_last = 0.0;
return {};
}
}
