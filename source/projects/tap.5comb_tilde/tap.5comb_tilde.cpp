...
class fivecomb : public object<fivecomb>, public vector_operator<> {
  ...
  public:
    ...
    fivecomb(const atoms& args = {}) { m_bank.prepare(samplerate()); }
...
#define TAP_5COMB_VOICE_ATTR(NAME, INDEX, DEFAULT, DESC)                                                               \
    ...
    TAP_5COMB_VOICE_ATTR(
        res1, kernel::p_res1 + 0, 50.0,
        "Resonance of comb 1 (0..100). 0 is none; 100 rings effectively forever. Maps to ring time on a log curve.")
    ...
    TAP_5COMB_VOICE_ATTR(
        lp1, kernel::p_lp1 + 0, 20000.0,
        "Cutoff (Hz) of the lowpass in comb 1's feedback loop (0..20000). Low values smooth/darken the ring; high values keep it bright and metallic.")
    TAP_5COMB_VOICE_ATTR(lp2, kernel::p_lp1 + 1, 20000.0,
                         "Cutoff (Hz) of the lowpass in comb 2's feedback loop (0..20000).")
    TAP_5COMB_VOICE_ATTR(lp3, kernel::p_lp1 + 2, 20000.0,
                         "Cutoff (Hz) of the lowpass in comb 3's feedback loop (0..20000).")
    TAP_5COMB_VOICE_ATTR(lp4, kernel::p_lp1 + 3, 20000.0,
                         "Cutoff (Hz) of the lowpass in comb 4's feedback loop (0..20000).")
    TAP_5COMB_VOICE_ATTR(lp5, kernel::p_lp1 + 4, 20000.0,
                         "Cutoff (Hz) of the lowpass in comb 5's feedback loop (0..20000).")

#undef TAP_5COMB_VOICE_ATTR

    // -- masters and globals ---------------------------------------------------------------------

    attribute<number> freq{this, "freq", 1.0, setter{ MIN_FUNCTION {
    ...
    attribute<number> res{this, "res", 1.0, setter{ MIN_FUNCTION {
    ...
    attribute<number> lp{this, "lp", 1.0, setter{ MIN_FUNCTION {
    ...
    attribute<number> warp{this, "warp", 0.0, setter{ MIN_FUNCTION {
    ...
    attribute<number> phase{this, "phase", 0.0, setter{ MIN_FUNCTION {
