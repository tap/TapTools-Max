...
inline double clamp_param(int index, double value) {
    switch (index) {
        ...
        case p_level_master:
        case p_fb_master:
        case p_delayscale:   return std::clamp(value, 0.0, 2.0);
        case p_rotate:       return std::clamp(value, -500.0, 500.0);
        case p_smear:        return std::clamp(value, 0.0, 100.0);
        case p_lforate:      return std::clamp(value, 0.0, 20.0);
        case p_lfodepth:     return std::clamp(value, 0.0, 100.0);
...
inline double clamp_curve(int which, double value) {
    switch (which) {
        case curve_level: return std::clamp(value, 0.0, 2.0);
        case curve_delay: return std::clamp(value, 0.0, k_max_delay_s * 1000.0);
        case curve_fb:    return std::clamp(value, 0.0, k_fb_curve_max);
        default:          return value;
    }
}
