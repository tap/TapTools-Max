...
class filecontainer : public object<filecontainer> {
  public:
    ...
    message<> removefile{
        this, "removefile", "Remove the named file from the container.",
        MIN_FUNCTION{
            if (args.empty()) {
                return {};
            }
    ...
    message<> getnumfiles{
        this, "getnumfiles", "Output the number of files stored in the container.",
        MIN_FUNCTION{
            mx::t_object* result = nullptr;
            sql_exec("SELECT filename FROM files", &result);
            const long n =
                result
                    ? static_cast<long>(reinterpret_cast<intptr_t>(mx::object_method(result, mx::gensym("numrecords"))))
                    : 0;
    ...
    message<> getfilenames{
        this, "getfilenames", "Output the names of the files stored in the container.",
        MIN_FUNCTION{
            mx::t_object* result = nullptr;
    ...
    message<> getfilepath{
        this, "getfilepath", "Output the temp path of a named extracted file.",
        MIN_FUNCTION{
            if (args.empty()) {
                return {};
            }
    ...
    message<> sql{
        this, "sql", "Run a raw SQL query on the container (do not SELECT the content column).",
        MIN_FUNCTION{
            if (args.empty()) {
                return {};
            }
    ...
    message<> addfileattr{
        this, "addfileattr", "Add a metadata attribute (name/value) to a named file.",
        MIN_FUNCTION{
            if (args.size() != 3) {
            ...
            sql_exec(sql, &result);
            char** record =
                result ? reinterpret_cast<char**>(mx::object_method(result, mx::gensym("nextrecord"))) : nullptr;
            if (record) {
                const atom& v = args[2];
    ...
    message<> getnumfileattrs{
        this, "getnumfileattrs", "Output the number of attributes for a named file.",
        MIN_FUNCTION{
            if (args.empty()) {
                return {};
            }
    ...
    message<> getfileattrnames{
        this, "getfileattrnames", "Output the attribute names for a named file.",
        MIN_FUNCTION{
            if (args.empty()) {
                return {};
            }
    ...
    message<> getfileattrvalue{
        this, "getfileattrvalue", "Output the value of a named attribute for a named file.",
        MIN_FUNCTION{
            if (args.size() != 2) {
  ...
  private:
    ...
    void restore_moddate(const char* filename, const char* moddate) {
        ...
        std::tm tm{};
        if (std::sscanf(moddate, "%4d-%2d-%2d %2d:%2d:%2d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour,
                        &tm.tm_min, &tm.tm_sec)
            != 6) {
            return;
        }
