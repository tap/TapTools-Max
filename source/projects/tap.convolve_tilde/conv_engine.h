...
///                 outL = inL ∗ h_LL + inR ∗ h_RL ;  outR = inL ∗ h_LR + inR ∗ h_RR
...
class conv_engine {
  public:
    ...
    void configure(int blocksize, int max_partitions) {
        m_B    = std::max(1, blocksize);
        m_N    = 2 * m_B;
        m_Pmax = std::max(1, max_partitions);

        const int flat = m_Pmax * m_N;
        for (int path = 0; path < k_paths; ++path) {
            for (int slot = 0; slot < 2; ++slot) {
                m_Hre[path][slot].assign(flat, 0.0);
                m_Him[path][slot].assign(flat, 0.0);
            }
        }
        for (int ch = 0; ch < k_channels; ++ch) {
            m_Xre[ch].assign(flat, 0.0);
            m_Xim[ch].assign(flat, 0.0);
            m_prev[ch].assign(m_B, 0.0);
            m_inblk[ch].assign(m_B, 0.0);
            m_outblk[ch].assign(m_B, 0.0);
        }
        m_fre.assign(m_N, 0.0);
        m_fim.assign(m_N, 0.0);
        m_are.assign(m_N, 0.0);
        m_aim.assign(m_N, 0.0);

        m_slotP[0] = m_slotP[1] = 0;
    ...
    void clear() {
        for (int ch = 0; ch < k_channels; ++ch) {
            std::fill(m_Xre[ch].begin(), m_Xre[ch].end(), 0.0);
            std::fill(m_Xim[ch].begin(), m_Xim[ch].end(), 0.0);
    ...
    int  block_size() const { return m_B; }
    int  max_partitions() const { return m_Pmax; }
    bool configured() const { return m_B > 0 && !m_Hre[0][0].empty(); }
    ...
    void load_ir(const float* const paths[k_paths], int length, double scale) {
        ...
        const int P = std::min(m_Pmax, (std::max(0, length) + m_B - 1) / m_B);

        for (int path = 0; path < k_paths; ++path) {
            const float* src = paths[path];
            double*      Hre = m_Hre[path][inactive].data();
            double*      Him = m_Him[path][inactive].data();
            ...
            for (int p = 0; p < P; ++p) {
                ...
                if (src) {
                    for (int j = 0; j < m_B; ++j) {
                        const int idx = p * m_B + j;
                        if (idx < length)
                            m_fre[j] = static_cast<double>(src[idx]) * scale;
                    }
                }
                fft(m_fre, m_fim, false);
                std::copy(m_fre.begin(), m_fre.end(), Hre + p * m_N);
                std::copy(m_fim.begin(), m_fim.end(), Him + p * m_N);
            }
        }

        m_slotP[inactive] = P;                                // written before the publish...
        m_active.store(inactive, std::memory_order_release);  // ...so (slot, P) stay consistent.
    }

    // Process n stereo samples. Wet (fully convolved) output is written to outL/outR. Safe for any n;
    // input/output must not alias each other.
    void process(const double* inL, const double* inR, double* outL, double* outR, long n) {
        for (long i = 0; i < n; ++i) {
            m_inblk[0][m_pos] = inL[i];
            m_inblk[1][m_pos] = inR[i];
            outL[i]           = m_outblk[0][m_pos];
            outR[i]           = m_outblk[1][m_pos];
            if (++m_pos == m_B) {
                process_block();
                m_pos = 0;
            }
  ...
  private:
    ...
    void process_block() {
        ...
        for (int ch = 0; ch < k_channels; ++ch) {
            for (int j = 0; j < m_B; ++j) {
                m_fre[j]       = m_prev[ch][j];
                m_fre[m_B + j] = m_inblk[ch][j];
                m_fim[j]       = 0.0;
                m_fim[m_B + j] = 0.0;
            }
            fft(m_fre, m_fim, false);
            std::copy(m_fre.begin(), m_fre.end(), m_Xre[ch].data() + cur * m_N);
            std::copy(m_fim.begin(), m_fim.end(), m_Xim[ch].data() + cur * m_N);
        ...
        for (int oc = 0; oc < k_channels; ++oc) {
            ...
            for (int ic = 0; ic < k_channels; ++ic) {
                ...
                const double* Hre  = m_Hre[path][a].data();
                const double* Him  = m_Him[path][a].data();
                const double* Xre  = m_Xre[ic].data();
                const double* Xim  = m_Xim[ic].data();

                for (int p = 0; p < P; ++p) {
                    int slot = cur - p;
                    if (slot < 0)
                        slot += m_Pmax;
                    const double* hr = Hre + p * m_N;
                    const double* hi = Him + p * m_N;
                    const double* xr = Xre + slot * m_N;
                    const double* xi = Xim + slot * m_N;
                    for (int k = 0; k < m_N; ++k) {
            ...
            for (int j = 0; j < m_B; ++j)
                m_outblk[oc][j] = m_are[m_B + j]; // overlap-save: discard the aliased first half
        }
    ...
    int m_B{0};    // partition (block) size = latency in samples
    int m_N{0};    // FFT size = 2 * m_B
    int m_Pmax{0}; // capacity in partitions

    std::atomic<int> m_active{-1};     // published IR slot (0/1), or -1 for none. Audio thread reads it.
    int              m_slotP[2]{0, 0}; // partition count per slot (kept consistent with m_active)

    std::array<std::array<std::vector<double>, 2>, k_paths> m_Hre, m_Him; // IR spectra [path][slot]
    std::array<std::vector<double>, k_channels>             m_Xre, m_Xim; // input FDL [channel]
    std::array<std::vector<double>, k_channels>             m_prev, m_inblk, m_outblk;

    std::vector<double> m_fre, m_fim, m_are, m_aim; // scratch

    int m_pos{0}; // fill/read index within the current block [0, m_B)
