...
class comb : public object<comb>, public vector_operator<> {
  public:
    ...
    attribute<number> buffersize{
        this, "buffersize", 200.0,
        description{"Size of the internal delay buffer in milliseconds (fixed at instantiation)."}};

    attribute<number> feedback{this, "feedback", 0.9, setter{MIN_FUNCTION{m_fb_coef = args[0];
...
attribute<bool> autoclip{this, "autoclip", false, setter{MIN_FUNCTION{m_clip = args[0] ? 1.0 : 0.0;
return args;
}
}
, description {
    "Clip the feedback signal to +/-1 to keep the filter from blowing up."
}
}
;

attribute<number> delay{this, "delay", 50.0, setter{MIN_FUNCTION{set_delay(args[0], true);
return {m_delay_ms};
}
}
, description {
...
attribute<number> decay{this, "decay", 0.0, setter{MIN_FUNCTION{m_decay = args[0];
...
, description {
    "Decay time in seconds (an alternative way to set the feedback)."
}
}
;

attribute<number> lowpass{this, "lowpass", 20000.0, setter{MIN_FUNCTION{m_lp_hz = args[0];
update_lowpass();
return args;
}
}
, description {
    "Cutoff frequency (Hz) of the lowpass filter in the feedback path."
}
}
;

message<> clear{this, "clear", "Clear the delay buffer and filter memory.", MIN_FUNCTION{reset();
return {};
}
}
;
...
message<> m_number{this, "number", "Set the delay (inlet 2) or decay (inlet 3).",
                   MIN_FUNCTION{if (inlet == 1){delay = args[0];
}
else if (inlet == 2) {
    decay = args[0];
}
...
message<> dspsetup{this, "dspsetup", "Allocate/clear and recompute when the DSP chain starts.", MIN_FUNCTION{allocate();
update_lowpass();
set_delay(m_delay_ms, false);
return {};
}
}
;
