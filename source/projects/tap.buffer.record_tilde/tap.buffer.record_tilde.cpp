...
;
argument<int> fade_arg{this, "fade", "Fade length in samples.", MIN_ARGUMENT_FUNCTION{fade = static_cast<int>(arg);
}
}
;

attribute<bool> loop{this, "loop", false, description{"Loop back to the start of the buffer when the end is reached."}};

attribute<int> fade{this, "fade", 5,
                    setter{MIN_FUNCTION{return {std::clamp(static_cast<int>(args[0]), 1, k_max_fade - 1)};
}
}
, description {
    "Length of the write fade window, in samples (1-255)."
}
}
;
...
message<> number{this, "number", "A non-zero int starts recording; 0 stops and rewinds.",
                 MIN_FUNCTION{m_record_toggle = static_cast<int>(args[0]);
...
message<> dspsetup{this, "dspsetup", "Clear the fade memory when the DSP chain starts.",
                   MIN_FUNCTION{m_increment_steps.fill(0.0);
m_loc = 0;
return {};
}
}
