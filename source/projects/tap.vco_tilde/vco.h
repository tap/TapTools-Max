...
inline double clamp_param(int index, double value) {
    switch (index) {
        ...
        case p_shape:     return std::clamp(value, 0.0, 3.0);
        case p_pw:        return std::clamp(value, 1.0, 99.0);
        case p_drift:     return std::clamp(value, 0.0, 100.0);
        case p_detune:    return std::clamp(value, -1200.0, 1200.0);
...
private:
    ...
    double saw_at(double p, double dt) const {
    ...
    double step(double base_hz, double fm_hz, double sync) {
        ...
        if (m_sync_prev <= 0.0 && sync > 0.0) {
            ...
            const double d     = waveform_out_peek(p_old, adt, shape, pw)
                               - waveform_out_peek(wrap01(p_new), adt, shape, pw);
