...
#define TAP_VCO_ATTR(NAME, INDEX, DEFAULT, DESC)                                                   \
    ...
    TAP_VCO_ATTR(pw, kernel::p_pw, 50.0,
        "Pulse width in percent (1..99), active as shape approaches 3.")
    TAP_VCO_ATTR(drift, kernel::p_drift, 0.0,
        "Analog-style slow random pitch drift depth in cents (0..100). Deterministic per seed.")
    TAP_VCO_ATTR(detune, kernel::p_detune, 0.0,
        "Static detune in cents (-1200..1200).")
    TAP_VCO_ATTR(gain, kernel::p_gain, 0.0,
        "Output gain in dB.")

#undef TAP_VCO_ATTR

    attribute<int> seed { this, "seed", 1,
        setter { MIN_FUNCTION {
    ...
    message<> waveform { this, "waveform",
        "Snap the shape to a classic waveform: sine, triangle, saw, or pulse.",
        MIN_FUNCTION {
            if (!args.empty()) {
    ...
    void operator()(audio_bundle input, audio_bundle output) override {
        ...
        for (long i = 0; i < n; ++i) {
            ...
            out[i] = has_freq ? m_osc.process_at(freq[i], fm_v, sync_v)
                              : m_osc.process(fm_v, sync_v);
        }
    }
