...
int main(int argc, char** argv) {
    ...
    // 1. PWM pad: two drifted, detuned pulses with a slow width sweep.
    {
        ...
        b.set_detune(9.0);
        const size_t n = static_cast<size_t>(10.0 * g_sr);
        ...
        write_wav(dir + "vco_pwm_pad.wav", out, g_sr);
    }
    ...
    // 3. Through-zero FM growl: modulation depth swept way past the carrier.
    {
        ...
        write_wav(dir + "vco_tzfm_growl.wav", out, g_sr);
    }
    ...
    // 4. The shape-morph tour: sine -> triangle -> saw -> pulse over 9 s.
    {
        ...
        for (size_t i = 0; i < n; ++i)
            out[i] = 0.4 * o.process();
        write_wav(dir + "vco_shape_morph.wav", out, g_sr);
    }
    ...
    // 5. Preset morph: airy drifted sine -> hard nasal pulse over 4 s, mid-note.
    {
        ...
        const size_t n = static_cast<size_t>(12.0 * g_sr);
        std::vector<double> out(n);
        for (size_t i = 0; i < n; ++i) {
            if (i == static_cast<size_t>(4.0 * g_sr))
                o.recall_preset(1, 4.0);
            out[i] = 0.4 * o.process();
        }
        write_wav(dir + "vco_preset_morph.wav", out, g_sr);
    }
