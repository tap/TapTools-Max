...
double goertzel(const std::vector<double>& x, size_t lo, size_t hi, double freq_hz) {
    const double w    = 2.0 * kvo::k_pi * freq_hz / c_sr;
...
SCENARIO("the shape morph glides without discontinuities") {
    ...
    double maxjump = 0.0;
    const double slope_bound = 2.0 * kvo::k_pi * 220.0 / c_sr * 1.2 + 0.05;  // sine slope + margin
    size_t discont = 0;
...
SCENARIO("hard sync locks the period to the master") {
    ...
    for (size_t i = 0; i < n; ++i) {
        const double sync = std::sin(2.0 * kvo::k_pi * master * i / c_sr);
...
SCENARIO("through-zero FM creates sidebands and survives deep modulation") {
    GIVEN("moderate FM") {
        ...
        std::vector<double> out(n);
        for (size_t i = 0; i < n; ++i)
            out[i] = o.process(depth * std::sin(2.0 * kvo::k_pi * fmod * i / c_sr));
    ...
    GIVEN("modulation depth far beyond the carrier (through zero)") {
        ...
        std::vector<double> out(n);
        for (size_t i = 0; i < n; ++i)
            out[i] = o.process(1500.0 * std::sin(2.0 * kvo::k_pi * 30.0 * i / c_sr));
