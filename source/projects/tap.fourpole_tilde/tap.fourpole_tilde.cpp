...
class fourpole : public object<fourpole>, public sample_operator<2, 2> {
  ...
  public:
    ...
    attribute<number> frequency{this, "frequency", 1000.0, setter{MIN_FUNCTION{m_frequency = args[0];
    update_coefficients();
    return args;
}
}
...
attribute<number> q{this, "q", 0.0, setter{MIN_FUNCTION{m_q = MIN_CLAMP(static_cast<double>(args[0]), 0.0, 1.0);
update_coefficients();
return args;
}
}
, description {
    "Resonance (0 = none, approaching 1 = self-oscillation)."
}
}
;
...
message<> dspsetup{this, "dspsetup", "Recompute coefficients for the current sample rate.",
                   MIN_FUNCTION{update_coefficients();
return {};
}
}
;
...
double tick(ladder& s, double x) {
    ...
    s.out1 = x + 0.3 * s.in1 + (1.0 - m_f) * s.out1;
    s.in1  = x;
    s.out2 = s.out1 + 0.3 * s.in2 + (1.0 - m_f) * s.out2;
    s.in2  = s.out1;
    s.out3 = s.out2 + 0.3 * s.in3 + (1.0 - m_f) * s.out3;
    s.in3  = s.out2;
    s.out4 = s.out3 + 0.3 * s.in4 + (1.0 - m_f) * s.out4;
    s.in4  = s.out3;
