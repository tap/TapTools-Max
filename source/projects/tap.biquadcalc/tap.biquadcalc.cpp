...
class biquadcalc : public object<biquadcalc> {
  ...
  public:
    ...
    biquadcalc(const atoms& = {}) { m_initialized = true; }

    attribute<symbol> filter_type{this, "type", "lowpass",
                                  range{"lowpass", "highpass", "bandpass", "bandstop", "peaknotch", "lowshelf",
                                        "highshelf", "resonant", "allpass"},
                                  description{"The kind of filter algorithm to use."}};

    attribute<number> cf{this, "cf", 1000.0, setter{MIN_FUNCTION{trigger_if(1);
    return args;
}
}
, description {
...
attribute<number> gain{this, "gain", 0.0, setter{MIN_FUNCTION{trigger_if(2);
return args;
}
}
, description {
    "Gain of the filter in decibels (where applicable to the filter type)."
}
}
;

attribute<number> q{this, "q", 1.0, setter{MIN_FUNCTION{trigger_if(3);
return args;
}
}
, description {
    "The Q (narrowness/steepness) of the filter."
}
}
;
...
attribute<int> trigger{this, "trigger", 4, range{0, 4},
                       description{"When the coefficients are recalculated: 0 = only on bang, 1 = cf, 2 = gain, "
                                   "3 = q, 4 = any of cf/gain/q."}};

message<> bang{this, "bang", "Calculate the coefficients and send them out the left outlet.", MIN_FUNCTION{calculate();
return {};
}
}
;
