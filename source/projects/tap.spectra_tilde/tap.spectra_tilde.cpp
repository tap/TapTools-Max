...
class spectra : public object<spectra>, public vector_operator<> {
  public:
    ...
    attribute<number> remap{this, "remap", 1.0, range{0.0, 16.0}, setter{MIN_FUNCTION{m_remap = args[0];
    return {m_remap};
}
}
, description {
    "Spectral remapping ratio. Output bin k takes its value from input bin "
...
message<> m_number{this, "number", "A float sets the remapping ratio.", MIN_FUNCTION{remap = args[0];
return {};
}
}
;
