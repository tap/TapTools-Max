...
class nr : public object<nr>, public vector_operator<> {
  public:
    ...
    attribute<number> threshold{this, "threshold", 0.01, range{0.0, 1.0}, setter{MIN_FUNCTION{m_threshold = args[0];
    return {m_threshold};
}
}
, description {
    "Linear-amplitude threshold. Bins below this level are attenuated; bins above "
    "it pass. 0 disables the gate (the output then reconstructs the input)."
}
}
;

attribute<number> slope{this, "slope", 2.0, range{0.0, 64.0}, setter{MIN_FUNCTION{m_slope = args[0];
return {m_slope};
}
}
, description {
    "How gradually the gate engages across the threshold (soft knee). Low values "
...
message<> m_number{this, "number",
                   ...
                   MIN_FUNCTION{if (inlet == 1){slope = args[0];
}
else {
    threshold = args[0];
}
...
message<> clear{this, "clear", "Reset the internal STFT buffers.", MIN_FUNCTION{reset_buffers();
return {};
}
}
;

message<> dspsetup{this, "dspsetup", "Reset buffers when the DSP chain starts.", MIN_FUNCTION{reset_buffers();
return {};
}
}
;
